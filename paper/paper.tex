...
Both signals are generated with SEOBNRv4.    We perform a multi-stage iterative RIFT analysis of these signal with the
...
$4096\unit{Hz}$ sampling and only the  $\ell=2, m=\pm 2$ modes.  We also conservatively report computational cost for
parameters $\lambda$ which are close to or within the posterior and therefore, due to their consistency with the data,
have the best-determined extrinsic parameters and thus converge the most slowly.

\editremark{report on distance to each signal, and in particular $d_{\rm max}$ as that might impact performance too}
chosen to have peak SNR $\simeq 14$ and 
...
\editremark{test on other hardware -- also problem, a lot of the speed improvement occurs because the initial grid is
  crap/too wide}
