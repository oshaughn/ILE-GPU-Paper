...
%
Previously, B. Miller et al \editremark{cite private communication} developed a custom-coded  implementation of the
relevant likelihood on graphics processing units (GPUs), suggesting substantial performance improvements.  
In this paper,  we describe a variant of the RIFT algorithm which flexibly translates its low-level
...
% COMMAND
%  plot_posterior_corner.py --posterior-file posterior_samples-4.dat  --posterior-file posterior_samples-3.dat --posterior-file posterior_samples-2.dat --posterior-file posterior_samples-1.dat --parameter m1 --parameter m2  --plot-1d-extra --ci-list  '[0.9]' --truth-file /home/oshaughn/unixhome/Projects/LIGO-ILE-Applications/ILE-GPU/ILE-GPU-Paper/demos/mdc.xml.gz  --composite-file all.net --quantiles None  --posterior-label 4 --posterior-label 3 --posterior-label 2 --posterior-label 1 --use-legend
%plot_posterior_corner.py --posterior-file posterior_samples-2.dat --posterior-file posterior_samples-1.dat --parameter m1 --parameter m2  --plot-1d-extra --ci-list  '[0.9]' --truth-file /home/oshaughn/unixhome/Projects/LIGO-ILE-Applications/ILE-GPU/ILE-GPU-Paper/demos/mdc.xml.gz  --composite-file all.net --quantiles None  --posterior-label 2 --posterior-label 1 --use-legend
...
2.5$ to $(5000/20/48)\simeq
 ...
 evaluation in the first requires roughly 60 seconds for both SpinTaylorT4 and SEOBNRv4ROM, on average.   Subsequent
marginalized likelihood iterations are not resource-limited and complete in roughly 35 minutes, depending on hardware.


\begin{figure*}
% COMMAND
% plot_posterior_corner.py  --posterior-file posterior_samples-3.dat --posterior-file posterior_samples-2.dat --posterior-file posterior_samples-1.dat --parameter mc --parameter delta_mc  --parameter chi_eff --mc-range  '[1.19655,1.197]' --chi-max 0.025 --plot-1d-extra --ci-list  '[0.9]' --truth-file /home/oshaughn/unixhome/Projects/LIGO-ILE-Applications/ILE-GPU/ILE-GPU-Paper/demos/mdc_bns.xml.gz  --composite-file all.net --quantiles None --posterior-label 3 --posterior-label 2 --posterior-label 1 --use-legend
...
Results for marginal posterior distributions
  ...
  show marginal CDFs and PDFs for the corresponding variable; and colored points indicate the location $\lambda$ and
  value of the underlying marginalized likelihood evaluations.  The solid curves show an analysis with SEOBNRv4\_ROM;
  the dotted curves show an analysis with SpinTaylorT4.  \editremark{currenly only v4ROM shown}  \emph{Left panel } Posterior distribution
  over  $\mc$,
...
binaries with suitable amplitudes and orientations, when circumstances allow.  These tighter constraints could
