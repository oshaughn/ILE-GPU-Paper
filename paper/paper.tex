...
\caption{\label{fig:BNS:MultiIterate}\textbf{Convergence of BNS analysis}: Results for marginal posterior distributions
  ...
  over  $\mc$ and
  $\delta=(m_1-m_2)/M$.    \emph{Center panel}: Marginal 1d CDFs of $\mc$, showing convergence.
\emph{Right panel}: Marginal 1d empirical CDF of $\ln{\cal L}_{\rm marg}$, showing that at late times the proposed
candidate points are consistent with the posterior (i.e., no proposed point has very low $\ln {\cal L}_{\rm marg}$.
}
\end{figure*}

% Code configuration
In this example, we use $100$ initial points spread over the two mass dimensions, adding $20$ evaluations per
iteration.  With one instance analyzing each 20 evaluations,  we use five cores in the first iteration, followed by one
core for remaining iterations.  We choose this simple configuration to facilitate visualization.    The left and center panels of Figure  \ref{fig:BNS:MultiIterate} show posterior disributions derived
from the first several RIFT iterations.  





* show convergence diagnostic of $\ln {\cal L}$ range over the data: can see few outliers.  Easy way to assess agreement
and stability

** can also do L2 difference between likelihoods estimated with different data
...
* COST OF OVERALL ANALYSIS, AVERAGED OVER WHOLE: average cost of $\ln {\cal L}$ is
4096 Hz is X.  First iteration averages about 30 minutes in this configuration, second and subsequent iterations require
about 2 h.
